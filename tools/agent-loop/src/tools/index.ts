...
// Convert a task id into the filesystem/branch-safe slug used for run artifacts.
...
// Force-remove a retained run worktree after successful apply.
...
export function captureCheckoutSnapshot(repoRoot: string, ignoredPaths: string[] = []): CheckoutSnapshot {
  ...
  const fingerprint = createHash("sha256")
    .update(JSON.stringify({ head, trackedDiff, untracked }))
    .digest("hex");
  return {
    head,
    fingerprint,
