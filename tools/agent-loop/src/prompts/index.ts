...
export function writeVerifierPrompt(promptFile: string, opts: VerifierPromptOptions): void {
  ...
  const content = [
    ...
    "Start with the supplied requirements, diff and check results. Identify concrete unresolved correctness questions, then batch only the source reads needed to answer them. Finish once each requirement is supported or a concrete gap is found.",
    "This is a fresh independent review, not a second implementation task. Do not reconstruct the executor conversation, design alternative implementations, or expand into unrelated repository cleanup. Judge the candidate and the actual assertions in its checks, not the executor's confidence.",
    ...
    "Use read/search tools for inspection and write for the result artifact. Ordinary native Pi review has no shell: the harness owns command execution. If supplied checks do not prove required behavior, return fail with the missing assertion instead of running new experiments.",
    "Task JSON:",
    taskJson,
    "",
    "Human gates:",
